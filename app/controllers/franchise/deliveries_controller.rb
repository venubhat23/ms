class Franchise::DeliveriesController < Franchise::BaseController
  ...
  def mark_delivered
    if @delivery.update(status: 'delivered', stage_updated_at: Time.current, stage_updated_by: current_user&.id)
  ...
  def mark_completed
    if @delivery.update(status: 'completed', stage_updated_at: Time.current, stage_updated_by: current_user&.id)
